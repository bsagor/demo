package sa.com.medisys.rufaida.activity.radiology;
...
public class RadiologyOrderActivity extends AppCompatActivity {
    final String TAG = "RadiologyOrderActivity";
    Context context;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_radiology_order);
        setTitle("Radiology Order");

        initialize();
    }

    private void initialize() {
        context = RadiologyOrderActivity.this;

        memory = new RufaidaPreferenceManager(getApplicationContext());
        user = memory.getPref(RufaidaPreferenceManager.KEY_USER_TYPE);

        tvSetProfile = (TextView) findViewById(R.id.tvSetProfile);
        edtPatientNo = (EditText) findViewById(R.id.edtPatientNo);

        if(user.equals("1")){
            tvSetProfile.setText("Patient Radiology Order");
        }
